...
class ClientProxy(IService):
    ...
    def __init__(self, sender, client):
        ...
        :type client: L{pycloudia.cluster.beans.Activity}
        """
        self.sender = sender
        self.client = client
    ...
    def authenticate(self, client_id, platform, access_token):
        ...
        response = yield self.sender.send_request_package(self.client, target, request)
        profile = AuthenticateResponseSchema().decode(response.content)
        return_value(profile)

    def _create_target(self, client_id):
        """
        :type client_id: C{str}
        :rtype: L{pycloudia.cluster.beans.Activity}
        """
        return Activity(service=self.service, runtime=client_id)
...
class ServerProxy(IServiceInvoker):
    ...
    def process_package(self, package):
        if COMMAND.AUTHENTICATE == package.headers[HEADER.COMMAND]:
